...
<v-card
  ...
  >
    ...
    <tbody>
      ...
      <tr v-if='utc' v-bind="setHistTime(utc)">
        <td style="font-size: 50% !important;"> prognoza</td>
        <td><v-icon style="padding-right: 5px;">{{WeatherIcon}} </v-icon><v-icon>{{WeatherArray}} </v-icon></td>
      </tr>
      <tr>
        <td style="font-size: 50% !important;">pomiar</td>
        <td style="font-size: 50% !important;">{{getLocalTime}}</td>
      </tr>
      <tr v-show='CheckWeatherHazards(measurement.ta.value,measurement.ra.value,measurement.ws.value)'> 
        <td><v-icon color="red">warning</v-icon></td>
        <td style="font-size: 50% !important;">{{hazardDescription}}</td>
...
export default {
 ...
 findPress0: function() {
    ...
    fetchMeasurements: function() {
      ...
      console.log("Measurements featched");
    },
  ...
  weather: function(press0,press1,press2,press3,press4,press5,press6,press7,press8,press9,press10,press11,press12){
      ...
      if ((currentpress >= 1006 && currentpress <= 1020) || currentpress < 1006)
        ...
        this.WeatherIcon = 'wi wi-day-sunny'; //temp
